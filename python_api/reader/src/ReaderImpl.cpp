...
namespace
{
  ...
  public:
        ...
        void toJson(bool strip, nlohmann::json &j, std::set<long> &done)
        {
          if (done.find(id) == done.end())
          {
            ...
            if (isRoot()) {
               ...
               node["group"] =  std::to_string(*procs.begin());
            } else {
              node["group"] = id;
            }
            ...
            for (auto &it : ch)
            {
              ...
              auto size = it.second->procs.size();
              link["value"] = size;
    ...
    public:
      ...
      void add(long id, const std::set<long> &procs)
      {
        Comm_ptr p = std::make_shared<Comm>(procs, id);
        for (auto &it : nodes)
        {
      ...
      long getCommForProcessor(long proc) const override {
          auto it = self_comms.find(proc);
          if (it != self_comms.end()) {
              return it->second;
          }
          return -1;
      }
...
#undef X
    ...
    public:
      ...
      virtual long getCommForProcessor(long proc) const override { return commMap->getCommForProcessor(proc);}
    ...
    class RootNode : public DataBaseImpl<IRootNode>
    {
      ...
      std::map<long, std::string> stdouts, stderrs;
    ...
    public:
      ...
      virtual std::shared_ptr<IMapNode> getPackages() override { INITMEMBER(packages, MapNode)
                                                                 return packages; }
      virtual std::shared_ptr<IMapNode> getActions() override { INITMEMBER(actions, MapNode)
                                                                return actions; }
      virtual std::shared_ptr<IArrayNode> getChildren() override { INITMEMBER(children, ArrayNode)
                                                                   return children; }
      virtual std::shared_ptr<ITestNode> getInitialization() override
      {
        INITMEMBER(initialization, TestNode)
        return initialization;
      }
      virtual std::shared_ptr<IArrayNode> getUnitTests() override { INITMEMBER(unitTests, ArrayNode)
                                                                    return unitTests; }
      virtual std::shared_ptr<IArrayNode> getLogs() override { INITMEMBER(logs, ArrayNode)
                                                               return logs; }
      virtual std::shared_ptr<IInfoNode> getInfoNode() override { INITMEMBER(infoNode, InfoNode)
                                                                  return infoNode; }
      virtual std::shared_ptr<IWorkflowNode> getWorkflowNode() override
      {
        INITMEMBER(workflowNode, WorkflowNode)
        return workflowNode;
      }
      virtual std::shared_ptr<ICommInfoNode> getCommInfoNode() override
      {
        INITMEMBER(commInfo, CommInfoNode)
        return commInfo;
      }
      ...
      virtual std::string getStdout(long processor, bool error) override {
      
        auto a = commInfo->getCommForProcessor(processor);
        auto &map = (error) ? stderrs : stdouts;
        auto it = map.find(a);
        if (it != map.end()) {
          return it->second;
        } 
      ...
      virtual void addStdout(long comm, std::string value, bool error ) {
        
        auto &map = (error) ? stderrs : stdouts;
        
        auto it = map.find(comm);
        if (it == map.end()) {
          map[comm] = value;
        } else {
          it->second += value;
...
#define QUEUESIZE 100
    ...
    Current<json> getLine() override
    {
      Current<json> res;
      res.setCurrent(nextCurr, nextValue);
      getLine_();
      return res;
    }
  ...
  template <typename N>
  using InMemoryParserVisitor = InMemoryParser::ParserVisitor<N>;
  template <typename N>
  using NoLockInMemoryParserVisitor = InMemoryParser::NoLockParserVisitor<N>;
  template <typename T, typename V>
  using InMemoryRootNodeWithThread = InMemoryParser::RootNodeWithThread<T, V>;
